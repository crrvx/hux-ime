...
- **字反查**：显示光标左侧 1 个字——上排（排头「咅」）= 拼音、 \
  下排（排头「虍」）= 虎码（多音 / 多码以 `/` 连接，缺数据 `?`）； \
  ←/→/↑/↓ 交应用处理（本层不消费；查码段不下发预编辑，避免 marked text 锁住光标）； \
  Esc / 再次触发 / 其它键退出；展示面为面板辅助文本条（auxUp/auxDown）。

  取字来源依序为：
  - ① 客户端上报的周边文本（fcitx5 `surroundingText()` 有效时）；
  - ② 客户端不上报时（终端等）经 AT-SPI 取焦点对象的文本与光标（见下条）；
  - ③ 都没有则两排为空、不提示。
- **AT-SPI 取字来源**（`fcitx5/shell/atspi_source.{h,cpp}`）：仅在上面的 ② 启用。 \
  可选构建（CMake `HUX_ATSPI=AUTO` / `ON` / `OFF`；`AUTO` 下缺 `atspi-2` 开发包即自动关闭， \
  关闭时只剩 ①③）。运行期在专用线程里同步轮询无障碍总线；按键路径只读缓存快照， \
  不做 D-Bus 往返、不阻塞输入循环。`atspi_init()` 惰性调用一次；必须设调用超时与 \
  遍历的节点数 / 时间预算上限。焦点对象用 `GetChildren` 有界递归 + `ATSPI_STATE_FOCUSED` 判定 \
  （registry 的全局缓存不可用）。`activate` / `deactivate` / `reset` 时作废缓存， \
  避免把上一个应用的文本当本应用的；`activate` 时客户端若不上报周边文本，还会预热一次查询 \
  （第一键也有提示）。快照只保留光标左侧 ≤64 字符，正文不进日志。 \
  总线不可达 / 无焦点对象 / 无 `Text` 接口 / 超预算 ⇒ 视同无来源（两排为空、不报错、不提示）。
