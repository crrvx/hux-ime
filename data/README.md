...
- `tiger_sentence.pinyin.bin.gz`：音反查索引（TCSRV01：音节表 + 拼写表（本体/缩写）+
  ...
  sha256 `18a0931a…`；源文件 `PY_c.dict.yaml` 22,749,557 字节，sha256 `f57fb9e9…`）；语义与接线见 [`../docs/design.md`](../docs/design.md)。
- `symbols.yaml`：标点表（`punctuator/half_shape|full_shape`；`{commit}`/标量/`{pair}`）。
  取自参照 `symbols.yaml`（主干 pin `9f742d2`；该文件自 `35a10b9` 以来未变，两个 pin 逐字节相同），仅覆盖一处默认：half_shape 的 `"/"` 提交 `"/"`
