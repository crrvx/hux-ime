...
- `tiger_sentence.reverse.bin.gz`：拼音反查索引（TCSRV01：音节表 + 拼写表（本体/缩写）+
  按码分组的词条）。由 `tools/gen_reverse_index.py` 从参照实现的 `PY_c.dict.yaml`（提交
  `898579f`，源自官方字词版/秃版小狼毫的简体拼音词典）转换而来；参数与校验和见
  [`../docs/REVERSE_INDEX_MANIFEST.json`](../docs/REVERSE_INDEX_MANIFEST.json)，
  语义与接线见 [`../docs/reverse-lookup.md`](../docs/reverse-lookup.md)。
