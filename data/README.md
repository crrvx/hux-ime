...
- `platform/fcitx5/CMakeLists.txt` 的 `install(FILES …)` 按它安装（**只用 `cmake --install`
  也能得到完整引擎**；见 [`../platform/README.md`](../platform/README.md)）；
...
- `tiger_sentence.codes.huma.txt`：**追加码表**——虎码官方版（`2026.08.15`）单字表里**主表没有
  ...
  扩展 A 6,592、扩展 B–G 74,942，另有官方表一并编码、落在 CJK 统一表意文字区段之外的 939 个
  符号：兼容汉字/部首/笔画/注音/假名等）。
  ...
  生成与登记规则见下方「追加码表」。
  两个开关决定这张表怎么装（默认都开，见 [`../docs/config.md`](../docs/config.md) 的「字集」分区，
  托盘「虎虚」子菜单里也能切、即时生效）：**启用全字集**关掉只装主表（15,369 条 / 9,794 字）；
  **过滤非汉字**开则丢掉上面那 939 条符号（默认装载 116,762 条 / 102,529 字，主表自带的 3 个
  标点/假名不受影响）。
- `tiger_sentence.lexical.bin`：紧凑词先验（TCSLEX01 Bloom filter）。
  来源、许可（CC BY 4.0）、参数与校验和见 [`../docs/resources.md`](../docs/resources.md) §1。
...
- 每张追加表的 BOM 会被逐表剥掉（`normalize_text_content` 只剥得掉合并内容最前面那个）；
  仓库里的码表仍保持无 BOM；
- 装载受两个运行时开关控制（默认都开；配置页「字集」分区与托盘「虎虚」子菜单都能改，改完重建词库
  即时生效）：`tiger_sentence_full_charset` 关掉即不装追加表，`tiger_sentence_filter_non_han`
  开则丢掉追加表里落在 CJK 统一表意文字区段之外的行（939 条）。
...
两条要知道的代价（现测数据：`huma` 表 102,332 条、出厂口径装载 116,762 条）：

- **装载成本**：release 库下仅主表约 17 ms / 常驻 19 MB；合并后满载约 140–156 ms / 常驻约 107 MB
  （实测峰值 170 MB；解析由约 50 ms 降到 11–18 ms）。每次 rebuild（含 `apply_high_freq_limit`
  与切这两个开关）都要重付——状态菜单切一次会阻塞约 0.15 s（debug 档约 1 s）。
- **学习指纹**：码表内容（合并后）进 `sentence-v2|rules=<hash>`，所以追加表一改，既有的学习记录
  就不再命中（库文件还在，属一次「学习失效」体感）；出厂口径下指纹与只有主表时**不同**，
  关掉「启用全字集」也会切到另一套分区。
