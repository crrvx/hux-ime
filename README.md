...
- `` ` `` 音反查（拼音 → 虎码）；`~` 字反查（光标左侧汉字 → 拼音 + 虎码）
- 托盘右键菜单（各条目与首项「虎虚」同级）：「提前上屏」子菜单（关闭 / 至输出 / 至预编辑串）、
  「标点映射」子菜单（关闭（半角）/ 全角（常用）/ 全角（all））、单字重码组句、数字直选、
  启用全字集、过滤非汉字、候选窗口显示预编辑、重新部署；模型信息并入首项
  （`虎虚：<模型格式名>` / `虎虚：无模型`；点击打开模型所在目录）
