...
### ②-补 AT-SPI 取字来源（客户端不上报周边文本时）

- **依据**：上游只用客户端上报的周边文本（fcitx5 `surroundingText()` 无效即无字可取）， \
  终端等不上报的应用里字反查没有来源；本仓改为「① 客户端上报 → ② 经 AT-SPI 取焦点对象文本 → \
  ③ 无」，并在 [`platform/README.md`](../platform/README.md) 立行为契约。
- **代价**：多一个**可选**系统依赖 `atspi-2`；启用时作为客户端读会话无障碍总线 \
  （只读焦点文本与光标；不写、不做输入注入、正文不进日志）；应用不暴露无障碍文本时仍取不到； \
  关掉构建（`HUX_ATSPI=OFF`）即完全回退上游行为。
- **可证伪表达**：不进金样比对——它只改平台层的**输入来源**，核心算法与差异表 `DEVIATIONS` 不变； \
  由平台层夹具 `tools/atspi/` 覆盖（取字、窗口上限、超时与降级），见 [`resources.md`](resources.md) §5。
