...
| 上翻页 / 下翻页 | `-/=` 或 `[/]` | 下翻页：有候选时生效；上翻页：**已翻过页后**生效（参照 `when: paging`）；PgUp/PgDn 始终可用。绑到此处的键在菜单可见时**优先翻页**（不被方案标点分支遮蔽——本仓有意偏离上游 `abad411`，见 [`refactor.md` §8](refactor.md)）；**未翻页的** `-` 等上翻页键不满足 `when: paging`，仍按上游行为确认组合后落标点 |
