...
| 上翻页 / 下翻页 | `-/=` 或 `[/]` | 下翻页：有候选时生效；上翻页：**已翻过页后**生效（参照 `when: paging`）；PgUp/PgDn 始终可用。**注意**：菜单可见时这些 ASCII 标点会先被方案处理器确认组合（`_auto_commit` 下即上屏）再交标点表，故该路径上的翻页绑定被遮蔽——追平上游 `abad411`「标点学习保留」的参照行为 |
