...
托盘右键菜单里各条目与首项「**虎虚**」同级（「虎虚」只有状态区图标与模型信息，无子菜单）： \
「**提前上屏**」子菜单（关闭 / 至输出 / 至预编辑串）、「**标点映射**」子菜单（关闭（半角）/ 全角（常用）/ 全角（all））、\
单字重码组句、数字直选（与配置页「行为」分区是同一批值）、\
启用全字集、过滤非汉字（与配置页「字集」分区是同一批值）\
...
此外还有「候选窗口显示预编辑」\
（写入 `conf/hux.conf`，切换即时生效）与「重新部署」（重读配置与选项存储、重装数据与模型、\
重置全部会话）。模型信息并入首项「虎虚」（`虎虚：<模型格式名>` / `虎虚：无模型`，\
**点击该项打开模型所在目录**；文件名与失败原因进 `hux:` 日志的 `model:` 行，见下）；\
两个子菜单的父项也带当前值（`提前上屏：<当前态>`、`标点映射：<当前值>`），随改动即时重算。
