...
| **K3**（进行中） | fcitx5 addon：注册、候选/预编辑/上屏、数据路径、选项、学习库 ✅；宿主编辑语义 ✅（⑥）；英文模式**不实现**（⑦a 已移除：英文输入交 fcitx5 切换输入法）；标点表 ✅（⑦b）；音查虎 ✅（⑧-1：TCSRV01 索引 + 翻译/接线/金样；默认 Alt+`;`）、字查音+虎 ✅（⑧-2：组合触发（默认 Alt+`'`）、排头「咅/虍」取光标左侧 1 字、←/→/↑/↓ 交应用且查码段不下发预编辑（光标随动不回弹）；仅单字符触发键给默认可上屏候选）；金样参照 pin 升至 `8b615235`（自动上屏对齐修复已迁移；learning 生成器确定性修复）；打包（⑨）、状态菜单待做 | 真机可用 |
| **K4** | 验收与打包 | 真机清单 + 性能/内存 |
...
- 配置：addon `Settings`（9 项：早提交三项、full_shape/ascii_punct、tab_learning、high_freq_limit、
  音查虎/字查音+虎键；快捷键用 fcitx5 按键录入控件；未显式提供的项跟随 fcitx5 全局设置）；
