...
| **K3**（进行中） | fcitx5 addon：注册、候选/预编辑/上屏、数据路径、选项、学习库 ✅；宿主编辑语义 ✅（⑥）；英文模式**不实现**（⑦a 已移除：英文输入交 fcitx5 切换输入法）；标点表 ✅（⑦b）；音查虎 ✅（⑧-1：TCSRV01 索引 + 翻译/接线/金样）、字查音+虎 ✅（⑧-2：surrounding text + auxDown + 前2后2 窗口与步长 2）；金样参照 pin 升至 `8b615235`（自动上屏对齐修复已迁移；learning 生成器确定性修复）；打包（⑨）、状态菜单待做 | 真机可用 |
| **K4** | 验收与打包 | 真机清单 + 性能/内存 |
