...
- 拼音族：音节切分 + 拼音词典 + 候选排序（独立 translator），仅共享会话 / 宿主链 / 学习机制；
- 数据需求：拼音词典与音节表（可用 `tools/generators/gen_pinyin_index.py` 从 `PY_c.dict.yaml`
  生成同一格式的索引）、双拼键位映射表、可选词先验（`data/tiger_sentence.lexical.bin` 同格式）。
